...
export class OpenAPIGeneratorMockJs {
    ...
    sampleFromSchema(schema, propsName?: string[]) {
        const localSchema = schema.$ref
            ? get(this.openAPI, schema.$ref.replace('#/', '').split('/'))
            : objectify(schema);
            ...
            for (const method in openAPI.paths[path]) {
                ...
                for (const code in api.responses) {
                    ...
                    const schema =
                        ...
                        inferSchema(response.content['application/json']);
                    if (schema) {
