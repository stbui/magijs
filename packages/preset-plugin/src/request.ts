...
import { getPluginManager } from './core/plugin';
  ...
  import { setRequestConfig } from '${winPath(dirname(require.resolve('@magijs/request/package')))}';`,
  ]);
  api.addEntryCode(() => [
  ...
  api.onGenerateFiles(() => {
    ...
    api.writeTmpFile({
      path: 'index.ts',
      content: `
export { request } from '${winPath(dirname(require.resolve('@magijs/request/package')))}';
export { useRequest } from '${winPath(dirname(require.resolve('ahooks/package')))}';
`,
    });
    ...
    api.writeTmpFile({
      path: 'types.d.ts',
      tpl: `export { ResponseError,Context } from '${winPath(dirname(require.resolve('@magijs/request/package')))}';`,
      context: {},
    });
  });
  api.chainWebpack(memo => {
    ...
    memo.resolve.alias.set(
      'ahooks',
      getUserLibDir({ library: 'ahooks' }) || dirname(require.resolve('ahooks/package.json'))
    );
