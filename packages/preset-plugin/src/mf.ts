...
export default function mf(api) {
  api.describe({
    key: 'mf',
    config: {
      schema(Joi) {
  ...
  api.modifyWebpackConfig(async (config, { webpack }) => {
    ...
    if (!isEmpty(exposes)) {
      ...
      addMFEntry(config, name, join(api.paths.absTmpPath, 'plugin-mf', mfSetupPathFileName));
    }
...
`;
  ...
  async function constructExposes() {
    ...
    for await (const dirent of dir) {
      if (dirent.isDirectory()) {
        exposes['./' + dirent.name] = winPath(join(api.paths.absSrcPath, 'exposes', dirent.name));
      } else {
  ...
  function changeUmiEntry(config: any) {
    ...
    const asyncEntryPath = winPath(join(api.paths.absTmpPath, 'plugin-mf', mfAsyncEntryFileName));
