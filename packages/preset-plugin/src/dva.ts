...
export default api => {
  const pkgPath = join(dirname(require.resolve('@umijs/plugins/package.json')), 'libs', 'dva.ts');
...
${
  api.config.dva?.immer
    ? `
import dvaImmer, { enableES5, enableAllPlugins } from '${winPath(require.resolve('dva-immer'))}';
`
    : ''
}
...
function isModelObject(node: t.Node) {
  return (
    ...
    node.properties.some(property => {
      return ['state', 'reducers', 'subscriptions', 'effects', 'namespace'].includes((property as any).key.name);
    })
  );
}
