  ...
  api.modifyConfig({
    ...
    fn: memo => {
      ...
      if (userConfig.srcTranspiler || userConfig.jsMinifier || userConfig.cssMinifier) {
        logger.fatal(
          `Manual configuration of ${['srcTranspiler', 'jsMinifier', 'cssMinifier']
            .map(i => chalk.yellow(i))
            .join(', ')} is not supported when ${chalk.cyan('legacy')} build mode enabled.`
        );
      ...
      logger.ready(
        `${chalk.cyan('legacy')} mode is enabled, we automatically modify the ${[
          'srcTranspiler',
          'jsMinifier',
          'cssMinifier',
        ]
          ...
          if (
            ...
            logger.warn(
              `Legacy browsers do not support ${chalk.yellow(
                'Top level await'
              )}, ensure you are not using both ${chalk.bold.red(`Top level sync import`)} and ${chalk.bold.red(
                'Async externalsType (e.g. script)'
              )}`
            );
          }
        }
